...
func TestPool_Reap_HealthCheckFailure_BelowThreshold(t *testing.T) {
	m := mocks.NewMockInstanceServer(t)
	m.On("GetInstancesFull", api.InstanceTypeContainer).Return([]api.InstanceFull{
		{
			Instance: api.Instance{
	...
	// pgrep exec fails (simulates timeout)
	execOp := mocks.NewMockOperation(t)
	execOp.On("WaitContext", mock.Anything).Return(fmt.Errorf("context deadline exceeded"))
	m.On("ExecInstance", "azp-agent-0", mock.Anything, mock.Anything).Return(execOp, nil)
	...
	// Run reap — should not reap because failure count is below threshold
	err = pool.Reap(context.Background())
	require.NoError(t, err)
...
func TestPool_Reap_HealthCheckFailure_ReachesThreshold(t *testing.T) {
	...
	for i := range maxHealthCheckFailures {
		...
		if i == maxHealthCheckFailures-1 {
			// On the last iteration, expect the reap
			stopOp := mocks.NewMockOperation(t)
			stopOp.On("WaitContext", mock.Anything).Return(nil)
			m.On("UpdateInstanceState", "azp-agent-0", mock.MatchedBy(func(req api.InstanceStatePut) bool {
				return req.Action == "stop" && req.Force == true
...
func TestPool_Reap_HealthCheckFailure_ResetsOnSuccess(t *testing.T) {
	m := mocks.NewMockInstanceServer(t)
	m.On("GetInstancesFull", api.InstanceTypeContainer).Return([]api.InstanceFull{
		{
			Instance: api.Instance{
